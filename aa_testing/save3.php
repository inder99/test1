...
*/
	if(!isset($message)) {
		...
		$query = "SELECT * FROM tpc_students where stu_email = '" . $_POST["stu_email"] . "'";
		$count = $db_handle->numRows($query);
		
		if($count==0) {
			$query = "INSERT INTO tpc_students (stu_email, stu_password,first_name, last_name,father_name,mother_name,dob, gender,mobile,rollno,course,branch,semester,dept,cgpa,be_pass_year,twelveth_marks,twelveth_year,tenth_marks,tenth_year,projects,address,reappear) VALUES
...
<?php } ?>
        ...
        <fieldset>        
            ...
            <textarea name="projects" rows="6" cols="50" value="<?php if(isset($_POST['projects'])) echo $_POST['projects']; ?>"  required>            
            </textarea>
            
            <label class="align_left">Address</label>                  
            <textarea name="address" rows="5" cols="50"  value="<?php if(isset($_POST['address'])) echo $_POST['address']; ?>"  required>
            </textarea>   
            
            <label class="align_left">Reappear</label>                                    
            <select name="reappear"  value="<?php if(isset($_POST['reappear'])) echo $_POST['reappear']; ?>" >
