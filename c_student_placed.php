<?php
    ...
    if(isset($_GET['company_id']) && !empty($_GET['company_id']) && isset($_GET['student_id']) && !empty($_GET['student_id']) && isset($_GET['company_name']) && !empty($_GET['company_name'])){
        ...
        $search = mysql_query("SELECT * FROM tpc_student_reg WHERE student_id='".$student_id."' AND company_id='".$company_id."' AND placed='N' GROUP BY student_id") or die(mysql_error()); 
        $match  = mysql_num_rows($search);        
        mysql_free_result($search);
        //echo $match;
        if($match==1){
                // We have a match, update the placed
            mysql_query("UPDATE tpc_student_reg SET placed='Y' WHERE student_id='".$student_id."' AND company_id='".$company_id."' AND placed='N'") or  die(mysql_error());
            echo '<div class="statusmsg">Student Placed Successfully</div>';            
